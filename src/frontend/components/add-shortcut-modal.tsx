...
export function AddShortcutModal({
  ...
  const [selectedTextManipulationType, setSelectedTextManipulationType] =
    useState<string>("");
  const [scriptPath, setScriptPath] = useState("");
  const [shortcutName, setShortcutName] = useState("");
  // Ensure initial state matches the updated ShortcutKey allowing nulls
  const [definedShortcut, setDefinedShortcut] =
    useState<LocalShortcutKey | null>({
  ...
  const handleNext = () => {
    ...
    if (currentStep < 3) {
      ...
      if (
        window.electron &&
        typeof window.electron.addShortcut === "function"
      ) {
        ...
        console.error(
          "addShortcut function not found on window.electron. Ensure it is exposed via preload script."
        );
